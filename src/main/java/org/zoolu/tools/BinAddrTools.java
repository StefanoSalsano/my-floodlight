...
public class BinAddrTools
{
   ...
   {  return Integer.toString(BinTools.uByte(b[off++]))+"."+Integer.toString(BinTools.uByte(b[off++]))+"."+Integer.toString(BinTools.uByte(b[off++]))+"."+Integer.toString(BinTools.uByte(b[off]));
   }
   ...
   {  StringBuffer sb=new StringBuffer();
      ...
      {  int res=len-i;
         sb.append(BinTools.asHex(buf,off+i,((res<4)?res:4)));
         sb.append(' ');
         //sb.append('\n');
      }
      return sb.toString();
   ...
   {  StringBuffer sb=new StringBuffer();
      sb.append(BinTools.asHex(buf,off,6));
      sb.append(' ').append(BinTools.asHex(buf,off+6,6));
      sb.append(' ').append(BinTools.asHex(buf,off+12,2));
      off+=14;
      for (; off<len; off+=4) sb.append(' ').append(BinTools.asHex(buf,off,((len-off)>4)? 4: len-off));
      return sb.toString();
   }
   ...
   {  int hlen=4*(buf[off]&0x0F);
      BinTools.intTo2Bytes(0,buf,off+10);
      int checksum=checksum(buf,off,hlen);
      BinTools.intTo2Bytes(checksum,buf,off+10);
   }
   ...
   {  BinTools.copyBytes(BinTools.hexStringToBytes(mac_addr),buf,off,6);
